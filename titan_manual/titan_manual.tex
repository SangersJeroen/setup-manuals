...
\begin{enumerate}
	...
	\item Press the eucentric focus button to reset all lens currents.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\linewidth, keepaspectratio]{resources/ronchs_and_probes.png}
	\caption{
		\textbf{A selection of ronchigrams and their resulting probes.}
		\textbf{a)} The image of a probe whose crossover is at the correct height but sufficiently astigmatic as to not produce a ronchigram, it can be recognised by its distinct striations that do not connect at the centre.
		\textbf{b)} Astigmatic but closed ronchigram, its probe is displayed as an inset. The condensor astigmatism can be observed in both the elliptical shape of the ronchigram and the uneven diffuse halo around the probe itself.
		\textbf{c)} A well-corrected ronchigram and its probe. When selecting the featureless part of the ronchigram (dotted circle) with an appropriately sized aperture the circular probe displayed in the inset is formed.
	}
	\label{figure:ronchs_and_probes}
\end{figure}
...
\subsection*{Low-loss EELS}
\subsection*{Core-loss EELS}
